const orderMailMessage = (order) => {
    ...
    const products = order.products.map((product, i) => (
        `<tr>
           ...
           <td style='border: 1px solid #333; text-align: center;'>
                <span style='background-color: ${COLORS_DATA[product.color].hex}; 
    ...
    <div style='border: 1px solid silver;
            ...
            <hr/>
            <div>
                <p>Покупець: <span style='font-weight: bold; margin-left: 5px;'>${order.customer.surname}, ${order.customer.name}</span></p>
                <p>Ел. пошта: <span style='font-weight: bold; margin-left: 5px;'>${order.customer.email}</span></p>
                <p>Телефон: <span style='font-weight: bold; margin-left: 5px;'>${order.customer.phone}</span></p>
                <br/>
                <p>Метод зв‘язку: <span style='font-weight: bold; margin-left: 5px;'>${order.connectionMethod}</span></p>
                <p>Спосіб доставки: <span style='font-weight: bold; margin-left: 5px;'>${order.delivery.method}</span></p>
                ${delivery()}         
                <hr/>
